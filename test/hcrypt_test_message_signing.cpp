...
namespace {
    ...
    void test_message_signing(int offset,
        ...
        try {
            ...
            printf("\n%*c-| Hash algorithm %S, encryption algorithm %S, key length %Iu |-\n",
                    offset,
                    ' ',
                    hash_algorithm_name,
            ...
            hcrypt::buffer signature{ k.sign_hash(hash_value.data(),
                                                  hash_value.size()) };
...
void test_message_signing() {
    try {
        ...
        bcrypt::generate_random(data_to_sign.data(),
                                data_to_sign.size());
        
        printf("%*c%ws\n", 
               offset + 2, 
               ' ', 
               hcrypt::to_hex(data_to_sign).c_str());
