...
void print_provider_interface_info(int offset, wchar_t const *provider_name, ULONG mode, ULONG itf_id) {
    ...
    try {
        bcrypt::provider_registration_cptr registration{
            bcrypt::query_provider_registartion(provider_name, mode, itf_id)};
        if (registration) {
    ...
    try {
        ...
        bcrypt::find_first(providers, [offset](wchar_t const *provider_name) -> bool {
            ...
            bcrypt::find_first_interface([offset, provider_name](ULONG itf_id) -> bool {
                print_provider_interface_info(offset + 2, provider_name, CRYPT_UM, itf_id);
                return true;
            });
            ...
            bcrypt::find_first_interface([offset, provider_name](ULONG itf_id) -> bool {
                print_provider_interface_info(offset + 2, provider_name, CRYPT_KM, itf_id);
                return true;
            });
        ...
        printf("print_registered_providers, error code = 0x%x, %s\n",
               ex.code().value(),
               ex.what());
    }
    printf("----------------\n");
