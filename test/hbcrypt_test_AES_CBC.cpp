...
namespace {
    ...
    void test_aes_cbc(int offset, 
                       wchar_t const *algorithm, 
                       wchar_t const *chain_mode) {
        try {
            ...
            bcrypt::key k_a{ ap.generate_symmetric_key(reinterpret_cast<char const *>(key128), 
                                                       16) };
            ...
            printf("%*cKey: %S\n",
                   offset,
                   ' ',
                   hcrypt::to_hex(exported_key).c_str());
            ...
            std::copy( std::begin(plain_text),
                       std::end(plain_text),
                       std::begin(data_buffer));
...
void test_aes_cbc() {
    try {
        ...
        std::for_each(std::begin(algorithms),
                      std::end(algorithms),
                      [offset](wchar_t const *algorithm) {
